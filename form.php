...
, $risks, $conA, $conB, $conC1, $conC2, $conC3, $desc, $benf, $reas, $file1, $file2, $file3, $file4, $file5, $file6, $ids_array) {
    ...
    <html>
        <head>
            ...
            <script src="js/saveApplicationAjax.js"></script> 
        ...
        <body>
            <div id="header">
                ...
                <div id="menu" id="top">
                    <ul>
                        <li id="active"><a href="viewAppPage.php">Dashboard</a></li>
        ...
        <div id="content">
            ...
            <div><p>Use this form for general research only. It is not for health or medical research.Use this form for initial approvals and protocol modifications. To renew an approval after one year, please use the Continuation form.Submit this completed form and our proposal with all required documents.</p></div> 
                ...
                <form id="myForm" name="irbForm" method="POST" onsubmit="return formValidation()" enctype="multipart/form-data" >
                    <fieldset class="row1">
                    ...
                    <fieldset class="row1">
                        ...
                        <div><b>Upload document: </b><input type="file" id="upload1" name="upload1" <?php
                            if ($file1 == '') {
                                echo "required";
                            }
                    ...
                    <fieldset class="row1">
                        ...
                        <br/>
                        <div>Does the research involve any of the following procedures? :</div><div><input type="checkbox" class="cb" value="1" name="procedure[]" <?php echo (in_array("1", $ids_array) ? 'checked' : ''); ?>>Deception of the participant? </div>
                        <div> <input type="checkbox" value="2" class="cb" name="procedure[]" <?php echo (in_array("2", $ids_array) ? 'checked' : ''); ?>> Punishment of the participant?</div>
                        <div><input type="checkbox" value="3" class="cb" name="procedure[]" <?php echo (in_array("3", $ids_array) ? 'checked' : ''); ?>> Materials commonly regarded as socially unacceptable such as pornography, inflammatory text, ethnic portrayals?</div>	
                        <div> <input type="checkbox" value="4" class="cb"name="procedure[]" <?php echo (in_array("4", $ids_array) ? 'checked' : ''); ?>> Any other procedure that might be considered to be an invasion of privacy?</div>
                        <div> <input type="checkbox" value="5" class="cb" name="procedure[]" <?php echo (in_array("5", $ids_array) ? 'checked' : ''); ?>> Disclosure of the name of individual participants?</div>
                        <div> <input type="checkbox" value="6" class="cb" name="procedure[]" <?php echo (in_array("6", $ids_array) ? 'checked' : ''); ?>> Any other physically invasive procedure?</div>
                        <br/>
                        ...
                        <div><textarea name="risks" cols="50" rows="4" <?php
                            ...
                            ?>><?php echo htmlspecialchars($risks); ?></textarea></div>  
                    </fieldset>
                    ...
                    <fieldset class="row1">
                        ...
                        <br/>
                        <div><b>Upload document: </b><input type="file" id="upload2" name="upload2" <?php
                            if ($file2 == '') {
                                echo "required";
                            }
                            ?>><b>Uploaded Document:<?php echo $file2 ?></b></div>
                        <div>If full proposal is not available, explain why this document is not available.</div>
                        <div><textarea name="reas" cols="50" rows="4" value="<?php echo $reas ?>" required><?php echo htmlspecialchars($numA); ?></textarea></div>
                        ...
                        <div><b>Upload document: </b><input type="file" id="upload3" name="upload3" <?php
                            if ($file3 == '') {
                                echo "required";
                            }
                            ?>><b>Uploaded Document:<?php echo $file3 ?></b></div> 
                        ...
                        <div><b>Upload document: </b><input type="file" id="upload4" name="upload4" <?php
                            if ($file4 == '') {
                                echo "required";
                            }
                            ?>><b>Uploaded Document:<?php echo $file4 ?></b></div>
                        ...
                        <div><b>Upload document: </b><input type="file" id="upload5" name="upload5" <?php
                            if ($file5 == '') {
                                echo "required";
                            }
                            ?>><b>Uploaded Document:<?php echo $file5 ?></b></div>
                        ...
                        <div><b>Upload document: </b><input type="file" id="upload6" name="upload6" <?php
                            if ($file6 == '') {
                                echo "required";
                            }
                            ?>><b>Uploaded Document:<?php echo $file6 ?></b></div>
                    </fieldset>
                    <button class="button" type="button" name='save' onclick='avoidValidation(); submitAjax();return false;' formnovalidate>Save progress</button>
                    <input class="button" type="submit" value="Submit For Review" name='submit' id="sos">
...
if (isset($_REQUEST['submit'])) {
    ...
    //first handle non file data first and send to database
    $result = $user1->submitApp($appid, $usertype, $date, $projtitle, $princinvest, $coprincinvest1, $princinvestdept, $princinvestphone, $email, $deadline, $grant, $exemption, $numA, $numB, $numC, $numD, $numF, $numG
            , $risks, $conA, $conB, $conC1, $conC2, $conC3, $desc, $benf, $reas, $ids, "SUBMITTED");
    $user1->addStatus($appid, $usercode);//add application status to database
    for ($x = 1; $x <= 6; $x++) {
        $fileid = "upload" . $x;
        // echo "$fileid";
    ...
    if ($result == false) {
        ...
        header("Location: appStatusPage.php");
    }
} else {
